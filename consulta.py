...
class Captura(object):
    ...
    def captura_datos(rut):
        ...
        start_time = time()
        print(f'EL RUT A CONSULTAR ES: {rut}')
        ...
        url=f'http://2captcha.com/in.php?key={captcha_key}&method={method}&googlekey={google_key}&pageurl={page_url}'
        resp = requests.get(url) 
        print(resp.text)
        if resp.text[0:2] != 'OK':
            quit('Error. Captcha is not received')
        captcha_id = resp.text[3:]
        fetch_url = f'http://2captcha.com/res.php?key={captcha_key}&action=get&id={captcha_id}'
        for i in range(1, 20):	
            sleep(5) # wait 5 sec.
            resp = requests.get(fetch_url)
            if resp.text[0:2] == 'OK':
                break
        print(f'g-recaptcha-response: {resp.text[3:]}')
